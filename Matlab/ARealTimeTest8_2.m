%% 虚拟麦轮小车可执行轨迹版本：栅格地图剪枝 + 局部RRT* + 8方向运动约束  加入改进模糊PID控制
%% 说明：
...
% ===== 电机控制仿真参数：模糊自适应 PID =====
CONTROL_DT = 0.10;              % 控制周期 s
MAX_CHASSIS_SPEED = 950;        % 小车最大平移速度 mm/s
ARRIVE_THRESHOLD = 280;          % 到达轨迹点阈值 mm

MAX_WHEEL_SPEED = 1400;          % 单轮最大线速度 mm/s
MAX_PWM = 100;                  % PWM 最大值，模拟 Arduino 中 0~100 速度指令
pwm_last = zeros(4,1);
PWM_RATE_LIMIT = 5;   % 每帧 PWM 最大变化量

% TT 减速直流电机简化一阶模型参数
MOTOR_TAU = 0.25;               % 电机响应时间常数，越小响应越快
...
% 初始化四路模糊自适应 PID 状态
pid_state = initFuzzyPIDState2(4);
...
if b <= size(q, 1)
    ...
    if dist_to_target < ARRIVE_THRESHOLD
    ...
    if dist_to_target > 1500
        target_speed = MAX_CHASSIS_SPEED;
    elseif dist_to_target > 800
        target_speed = 0.90 * MAX_CHASSIS_SPEED;
    elseif dist_to_target > 300
        target_speed = 0.70 * MAX_CHASSIS_SPEED;
    elseif dist_to_target > ARRIVE_THRESHOLD
        target_speed = 0.45 * MAX_CHASSIS_SPEED;
    else
        target_speed = 0;
    end

    vx_ref = direction_vec(1) * target_speed;   % X+：右移
    vy_ref = direction_vec(2) * target_speed;   % Y+：前进
    wz_ref = 0;
    ...
    % ===== 3. 四个电机分别进行模糊自适应 PID 控制 =====
        ...
        for i = 1:4
            [pwm_pid(i), pid_state(i)] = fuzzyAdaptivePIDCorrection2(...
                motor_speed_ref(i), motor_speed_actual(i), pid_state(i), CONTROL_DT);
        end
        
        % 限制 PID 修正幅度，防止破坏麦轮速度分配关系
        PID_CORRECTION_LIMIT = 5;
    ...
    % ===== 4. TT 直流减速电机简化模型 =====
    motor_speed_actual = simulateTTMotor(...
        motor_speed_actual, pwm_cmd, CONTROL_DT, MOTOR_TAU, MOTOR_GAIN);

    % ===== 5. 四轮实际速度 -> 小车实际速度 =====
    [vx_real, vy_real, wz_real] = mecanumForwardKinematics(motor_speed_actual);

    % ===== 记录 PID 控制结果，用于实时显示 =====
    pid_time_log = [pid_time_log; frame_count];
    ...
    fprintf('[MOTOR PID] b=%d/%d target=(%.0f,%.0f) pos=(%.0f,%.0f) dist=%.1f vx=%.1f vy=%.1f\n', ...
