%% 最新的 虚拟麦轮小车可执行轨迹版本：栅格地图剪枝 + 局部RRT* + 8方向运动约束  加入该进模糊PID控制
%% 说明：
...
experiment_success = experiment_success || final_position_error <= GOAL_SUCCESS_THRESHOLD;
navigation_time = control_time_elapsed;
if isinf(min_obstacle_distance)
    min_obstacle_distance_print = NaN;
else
    min_obstacle_distance_print = min_obstacle_distance;
end
if isempty(replan_time_log_ms)
    mean_replan_time_ms = NaN;
    max_replan_time_ms = NaN;
else
    mean_replan_time_ms = mean(replan_time_log_ms);
    max_replan_time_ms = max(replan_time_log_ms);
end

fprintf('\n================ Experiment 1 Results ================\n');
fprintf('Success: %d\n', experiment_success);
fprintf('Final position error: %.1f mm\n', final_position_error);
fprintf('Minimum obstacle distance: %.1f mm\n', min_obstacle_distance_print);
fprintf('Replan attempts: %d\n', replan_count);
fprintf('First path conflict time: %.2f s\n', first_path_conflict_time);
fprintf('Replan computation time: mean %.1f ms, max %.1f ms\n', ...
    mean_replan_time_ms, max_replan_time_ms);
fprintf('Actual trajectory length: %.1f mm\n', path_length_actual);
fprintf('Navigation time: %.2f s\n', navigation_time);
fprintf('Frames: %d  |  Average FPS: %.2f\n', frame_count, average_fps);
fprintf('Average module time per frame: fetch %.1f ms, laser %.1f ms, mapping %.1f ms, grid %.1f ms, collision %.1f ms, control %.1f ms, visual %.1f ms\n', ...
    t_fetch/max(frame_count,1), t_laser/max(frame_count,1), t_map/max(frame_count,1), ...
    t_grid/max(frame_count,1), t_coll/max(frame_count,1), t_ctrl/max(frame_count,1), t_vis/max(frame_count,1));
fprintf('======================================================\n\n');
