...
HIT_THRESHOLD     = 2;       % 主循环中：命中≥N次才确认障碍（初次规划绕过此阈值）
DECAY_FRAMES      = 60;      % 超过 N 帧未被命中则衰减 1
DECAY_INTERVAL    = 10;      % 每 N 帧执行一次衰减检查
...
% ===== 初始化栅格地图 =====
% grid_hit_count(i,j)  = 该栅格累计被激光命中次数（i=x方向，j=y方向）
% grid_last_seen(i,j)  = 该栅格最后一次被命中的帧号（用于衰减）
...
function [grid_hit, grid_seen] = updateGridMap(grid_hit, grid_seen, xy_pts, ...
    ...
    % 过滤越界点（ix 对应 x 方向最大格数，iy 对应 y 方向最大格数）
