...
imwrite(image,'1.jpg');
imwrite(image1,'2.jpg');
%img1 = las_segm(image);
img=LaserFind3_1(image);% LaserFind使用霍夫变换寻找直线，LaserFind2
img1 = LaserFind3_1(image1);
...
las_dist = 135; % Laser Plane parameters
...
CVsyst_x1 = 93; % CV System second position 在unity中为CVSystemOrigin2的位置参数z*1000
...
las_dist1 = 138; % Laser Plane parameters
% Mapping
...
% 提取满足条件的数据 (10 < x < 16)
filteredData = mergedData(mergedData(:,1) > 10 & mergedData(:,1) < 25 & ...
...
%寻找偏移角度
[Y1max,Y1maxRow]=max(filteredData(:,2));%寻找y最大值
X1value = filteredData(Y1maxRow,1);
[X3max,X3Row]=max(filteredData(:,1));%寻找x最大值
Y3value = filteredData(X3Row,2);
[X2min,X2Row]=min(filteredData(:,1));%寻找x最小值
Y2value = filteredData(X2Row,2);
[Y4min,Y4minRow]=min(filteredData(:,2));%寻找y最小值
X4value = filteredData(Y4minRow,1);
%寻找立方体中心点
...
disp(['x 的平均值: ', num2str(averageX)]);
disp(['y 的平均值: ', num2str(averageY)]);

output=[averageX,averageY,thetha6];
end