%% 激光建图 + HC-08 全向小车 联合避障测试(A4:激光扫描里程融合)
...
currentPos = [0, 0];
currentYaw = 0;                    % 世界坐标航向角,逆时针为正(rad)

%% ===== A4 激光扫描里程计 =====
% 只改变定位点集，不改变激光提取和规划障碍点。固定地面横线及退化单直线不参与ICP。
lo.enabled = true;
lo.gridResolution = 25;            % 定位点云栅格降采样(mm)
lo.minPoints = 10;
lo.minSpan = 70;                   % x/y均需有足够跨度，排除单条线
lo.minGeometryRatio = 0.035;       % 点集协方差小/大特征值比
lo.dominantBandWidth = 20;         % 固定地面激光横带检测宽度(mm)
lo.dominantBandFraction = 0.40;
lo.dominantBandMinSpan = 140;
lo.maxCorrespondence = 90;         % ICP最近邻门限(mm)
lo.maxIterations = 12;
lo.trimFraction = 0.75;
lo.minInliers = 8;
lo.minInlierRatio = 0.45;
lo.maxRmse = 32;                   % ICP接受阈值(mm)
lo.maxTranslation = 180;           % 单帧最大可信位移(mm)
lo.maxRotationDeg = 7;
lo.stationaryTolerance = 25;      % 停车时拒绝较大的伪位移(mm)
lo.maxPredictionDisagreement = 140;
lo.fusionGain = 0.70;              % 0=仅电机，1=完全采用激光ICP
lo.runSelfTest = true;
scanOdomState = struct('hasPrevious', false, 'previousPoints', zeros(0,2));
odomDebug = emptyLaserOdometryObservation();
if lo.runSelfTest
    assertLaserOdometrySelfTest(lo);
end
...
fprintf('[INIT] A4激光里程: ICP=%d gain=%.2f grid=%dmm minPoints=%d rmse<=%.0fmm\n', ...
    lo.enabled, lo.fusionGain, lo.gridResolution, lo.minPoints, lo.maxRmse);
...
while isvalid(fig)
    try
        ...
        % A4:先用当前毫米点云修正位姿，再将规划障碍转换到世界坐标。
        % 定位点集和规划点集相互独立，ICP过滤不会删除规划障碍。
        localObstacles = [map_x(:), map_y(:)];
        captureElapsed = max(toc(motionClock), 0);
        predictedBodyAtCapture = estimateStateDisplacement( ...
            lastMotionState, lastMotionSpeed, captureElapsed, pose, S);
        odomPoints = prepareLaserOdometryPoints(localObstacles, lo);
        [scanOdomState, laserObservation] = updateLaserOdometry( ...
            scanOdomState, odomPoints, predictedBodyAtCapture, lo);
        [currentPos, currentYaw, motionClock, odomDebug] = updateFusedPose( ...
            currentPos, currentYaw, motionClock, lastMotionState, ...
            lastMotionSpeed, pose, S, laserObservation, lo);

        worldObstacles = bodyPointsToWorld(localObstacles, currentPos, currentYaw);
        ...
        plannerLocalObstacles = worldPointsToBody(plannerWorldObstacles, currentPos, currentYaw);
        ...
        if pp.enabled && replanDue && ...
            ...
            [currentPos, currentYaw, motionClock] = updateFusedPose( ...
                currentPos, currentYaw, motionClock, lastMotionState, ...
                lastMotionSpeed, pose, S, emptyLaserOdometryObservation(), lo);
            ...
            if plannerCanStart
                worldGoal = bodyPointsToWorld(pp.goal, currentPos, currentYaw);
        ...
        rawPathLocal = worldPointsToBody(rawDisplayPath, currentPos, currentYaw);
        execPathLocal = worldPointsToBody(remainingPath, currentPos, currentYaw);
        pathTarget = choosePathTarget(remainingPath, currentPos, currentYaw);
        ...
        if isgraphics(ax_plan) && ...
            ...
            title(ax_plan, sprintf('Local path | %s | odom=%s yaw=%.1f | pos=(%.0f, %.0f) grid=%d', ...
                char(dbg.planStatus), char(odomDebug.source), rad2deg(currentYaw), ...
                currentPos(1), currentPos(2), size(plannerWorldObstacles,1)), ...
                'Interpreter', 'none');
        end
        ...
        if mod(frame_count, 10) == 0
            fprintf(['[FRAME %d] action=%s plan=%s sd=%.0f state=%d wheels=%d speed=%d ' ...
                ...
                'odom=%s yaw=%.1f scan=%d icpN=%d rmse=%.1f\n'], ...
                ...
                char(odomDebug.source), rad2deg(currentYaw), ...
                odomDebug.scanPoints, odomDebug.inliers, odomDebug.rmse);
        end
...
function [position, yaw, clock, debug] = updateFusedPose( ...
        position, yaw, clock, state, speed, pose, S, observation, lo)
%UPDATEFUSEDPOSE Fuse motor prediction with a scale-aware laser scan correction.
    elapsed = max(toc(clock), 0);
    predictedBody = estimateStateDisplacement(state, speed, elapsed, pose, S);
    fusedBody = predictedBody;
    yawDelta = 0;
    debug = observation;
    debug.source = "motor";
    if startsWith(observation.source, "laser_")
        debug.source = "motor_" + erase(observation.source, "laser_");
    end

    if lo.enabled && observation.valid
        correction = observation.deltaBody - observation.predictedBody;
        fusedBody = predictedBody + lo.fusionGain * correction;
        yawDelta = lo.fusionGain * observation.deltaYaw;
        debug.source = "laser_fused";
    end

    position = position + bodyVectorsToWorld(fusedBody, yaw);
    yaw = atan2(sin(yaw + yawDelta), cos(yaw + yawDelta));
    debug.fusedBody = fusedBody;
    debug.yaw = yaw;
    clock = tic;
end
...
function target = choosePathTarget(remainingPath, currentPos, currentYaw)
%CHOOSEPATHTARGET Follow the immediate executable segment without cutting corners.
    ...
    target = worldPointsToBody(remainingPath(2,:), currentPos, currentYaw);
    if norm(target) < 1e-6
        target = [];
    end
end
...
function assertLaserOdometrySelfTest(lo)
%ASSERTLASERODOMETRYSELFTEST Verify ICP direction, scale, and ground-line rejection.
    [gx, gy] = meshgrid(-120:40:120, 80:40:280);
    previous = [gx(:), gy(:)];
    expectedTranslation = [32, 54];
    expectedYaw = deg2rad(3);
    current = (previous - expectedTranslation) * ...
        rotationMatrix2D(expectedYaw).';
    [translation, yaw, metrics] = registerLaserScans2D( ...
        previous, current, [30, 50], lo);
    assert(norm(translation - expectedTranslation) < 2.0 && ...
        abs(rad2deg(yaw - expectedYaw)) < 0.5 && metrics.rmse < 2.0, ...
        'A4激光ICP自检失败:坐标方向或尺度不一致。');

    groundLine = [(-200:20:200).', 200 * ones(21,1)];
    filtered = prepareLaserOdometryPoints(groundLine, lo);
    assert(isempty(filtered), 'A4激光ICP自检失败:固定地面横线未被剔除。');
end
function observation = emptyLaserOdometryObservation()
%EMPTYLASERODOMETRYOBSERVATION Typed default for motor-only fallback.
    observation = struct('valid', false, 'source', "motor", ...
        'deltaBody', [0, 0], 'predictedBody', [0, 0], ...
        'deltaYaw', 0, 'rmse', inf, 'scanPoints', 0, 'inliers', 0, 'inlierRatio', 0, ...
        'geometryRatio', 0, 'fusedBody', [0, 0], 'yaw', 0);
end

function points = prepareLaserOdometryPoints(localObstacles, lo)
%PREPARELASERODOMETRYPOINTS Build an ICP-only cloud without changing planning data.
    points = safeObstacles(localObstacles);
    if isempty(points)
        return;
    end
    visible = abs(points(:,1)) <= 450 & points(:,2) >= -50 & points(:,2) <= 750;
    points = points(visible, :);
    if isempty(points)
        return;
    end

    % A dense horizontal return is normally the laser/ground reference fixed
    % in the vehicle frame. It is useful for ranging but invalid for odometry.
    bands = round(points(:,2) / lo.dominantBandWidth);
    [~, ~, bandIndex] = unique(bands);
    bandCounts = accumarray(bandIndex, 1);
    [peakCount, peakBand] = max(bandCounts);
    inPeak = bandIndex == peakBand;
    if peakCount / size(points,1) >= lo.dominantBandFraction && ...
            range(points(inPeak,1)) >= lo.dominantBandMinSpan
        points = points(~inPeak, :);
    end
    if isempty(points)
        return;
    end

    cells = round(points / lo.gridResolution);
    [~, ~, cellIndex] = unique(cells, 'rows');
    points = [accumarray(cellIndex, points(:,1), [], @median), ...
              accumarray(cellIndex, points(:,2), [], @median)];
end

function [state, observation] = updateLaserOdometry( ...
        state, currentPoints, predictedBody, lo)
%UPDATELASERODOMETRY Estimate the vehicle transform from current scan to previous scan.
    observation = emptyLaserOdometryObservation();
    observation.predictedBody = predictedBody;
    observation.scanPoints = size(currentPoints,1);
    [geometryRatio, spans] = scanGeometryQuality(currentPoints);
    observation.geometryRatio = geometryRatio;

    geometryValid = size(currentPoints,1) >= lo.minPoints && ...
        all(spans >= lo.minSpan) && geometryRatio >= lo.minGeometryRatio;
    if ~lo.enabled || ~geometryValid
        state.hasPrevious = false;
        observation.source = "laser_rejected_geometry";
        return;
    end
    ...
    if ~state.hasPrevious
        ...
        observation.source = "laser_initializing";
        return;
    end

    [deltaBody, deltaYaw, metrics] = registerLaserScans2D( ...
        state.previousPoints, currentPoints, predictedBody, lo);
    state.previousPoints = currentPoints;
    observation.deltaBody = deltaBody;
    observation.deltaYaw = deltaYaw;
    observation.rmse = metrics.rmse;
    observation.inliers = metrics.inliers;
    observation.inlierRatio = metrics.inlierRatio;

    stationaryConflict = norm(predictedBody) < 5 && ...
        norm(deltaBody) > lo.stationaryTolerance;
    predictionConflict = norm(deltaBody - predictedBody) > ...
        lo.maxPredictionDisagreement;
    observation.valid = metrics.inliers >= lo.minInliers && ...
        metrics.inlierRatio >= lo.minInlierRatio && ...
        metrics.rmse <= lo.maxRmse && ...
        norm(deltaBody) <= lo.maxTranslation && ...
        abs(rad2deg(deltaYaw)) <= lo.maxRotationDeg && ...
        ~stationaryConflict && ~predictionConflict;
    if observation.valid
        observation.source = "laser_valid";
    else
        observation.source = "laser_rejected_fit";
    end
end

function [translation, theta, metrics] = registerLaserScans2D( ...
        previousPoints, currentPoints, initialTranslation, lo)
%REGISTERLASERSCANS2D Trimmed point-to-point ICP constrained to SE(2).
    translation = initialTranslation(:).';
    theta = 0;
    selected = false(size(currentPoints,1),1);
    matchIndex = ones(size(currentPoints,1),1);

    for iteration = 1:lo.maxIterations
        oldTranslation = translation;
        oldTheta = theta;
        rotation = rotationMatrix2D(theta);
        transformed = currentPoints * rotation + translation;
        distanceSquared = (transformed(:,1) - previousPoints(:,1).').^2 + ...
            (transformed(:,2) - previousPoints(:,2).').^2;
        [nearestSquared, matchIndex] = min(distanceSquared, [], 2);
        candidates = find(nearestSquared <= lo.maxCorrespondence^2);
        if numel(candidates) < lo.minInliers
            break;
        end
        [~, order] = sort(nearestSquared(candidates), 'ascend');
        keepCount = max(lo.minInliers, floor(lo.trimFraction * numel(candidates)));
        candidates = candidates(order(1:min(keepCount, numel(order))));
        selected(:) = false;
        selected(candidates) = true;

        [rotation, translation] = fitRigidTransform2D( ...
            currentPoints(selected,:), previousPoints(matchIndex(selected),:));
        theta = atan2(rotation(1,2), rotation(1,1));
        if norm(translation - oldTranslation) + ...
                50 * abs(atan2(sin(theta-oldTheta), cos(theta-oldTheta))) < 0.25
            break;
        end
    end

    if ~any(selected)
        metrics = struct('rmse', inf, 'inliers', 0, 'inlierRatio', 0);
        return;
    end
    rotation = rotationMatrix2D(theta);
    residual = currentPoints(selected,:) * rotation + translation - ...
        previousPoints(matchIndex(selected),:);
    metrics.rmse = sqrt(mean(sum(residual.^2, 2)));
    metrics.inliers = nnz(selected);
    metrics.inlierRatio = metrics.inliers / max(size(currentPoints,1), 1);
end

function [rotation, translation] = fitRigidTransform2D(moving, fixed)
%FITRIGIDTRANSFORM2D Least-squares rigid transform for row-vector points.
    movingCenter = mean(moving, 1);
    fixedCenter = mean(fixed, 1);
    centeredMoving = moving - movingCenter;
    centeredFixed = fixed - fixedCenter;
    [u, ~, v] = svd(centeredMoving.' * centeredFixed);
    rotation = u * v.';
    if det(rotation) < 0
        u(:,end) = -u(:,end);
        rotation = u * v.';
    end
    translation = fixedCenter - movingCenter * rotation;
end

function [ratio, spans] = scanGeometryQuality(points)
%SCANGEOMETRYQUALITY Reject point sets that constrain only one direction.
    if size(points,1) < 3
        ratio = 0;
        spans = [0, 0];
        return;
    end
    spans = max(points, [], 1) - min(points, [], 1);
    covariance = cov(points);
    eigenvalues = sort(max(real(eig(covariance)), 0));
    ratio = eigenvalues(1) / max(eigenvalues(end), eps);
end

function worldPoints = bodyPointsToWorld(bodyPoints, position, yaw)
    if isempty(bodyPoints)
        worldPoints = zeros(0, 2);
    else
        worldPoints = bodyPoints * rotationMatrix2D(yaw) + position;
    end
end

function worldVectors = bodyVectorsToWorld(bodyVectors, yaw)
    worldVectors = bodyVectors * rotationMatrix2D(yaw);
end

function bodyPoints = worldPointsToBody(worldPoints, position, yaw)
    if isempty(worldPoints)
        bodyPoints = zeros(0, 2);
    else
        bodyPoints = (worldPoints - position) * rotationMatrix2D(yaw).';
    end
end

function rotation = rotationMatrix2D(theta)
%ROTATIONMATRIX2D Row-vector rotation; positive theta turns vehicle left.
