%% 麦轮小车可执行轨迹版本：栅格地图剪枝 + 局部RRT* + 8方向运动约束  加入模糊PID控制
%% 说明：
...
MAX_CHASSIS_SPEED = 450;        % 小车最大平移速度 mm/s
ARRIVE_THRESHOLD = 180;          % 到达轨迹点阈值 mm

MAX_WHEEL_SPEED = 700;          % 单轮最大线速度 mm/s
...
if b <= size(q, 1)
    ...
    % ===== 3. 四个电机分别进行模糊自适应 PID 控制 =====
        ...
        for i = 1:4
            [pwm_pid(i), pid_state(i)] = fuzzyAdaptivePIDCorrection(...
