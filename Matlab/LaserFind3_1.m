%使用中心寻找法,直接使用HSV图像计算,只选取亮度阈值的提取结果

function img=LaserFind3_1(image)
...
upper_red5 = [0.2, 0.3, 1];%[0.2, 0.5, 1];
...
% 设置要保留的最小和最大像素数量（根据需要调整）
minPixelCount5 = 60;   % 保留的最小尺寸
maxPixelCount5 = 900; % 保留的最大尺寸
...
combinedImage =  cleanedImage5| cleanedImage6;
% 对掩膜进行形态学操作（膨胀和腐蚀）
redMaskC= imdilate(combinedImage, se);
combinedImage = imerode(redMaskC, se);
% 显示叠加后的图像
%figure;
